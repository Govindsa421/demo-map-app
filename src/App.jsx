...
const MapApp = () => {
  ...
  useEffect(() => {
    ...
    // Customizing Mapbox Draw styles to fix the error
  ...
  const handleDropdownToggle = (index) => {
    setActiveDropdown(activeDropdown === index ? null : index) // Toggle dropdown for current index
  }
              ...
              <tr key={idx} className='border-b'>
                ...
                <td className='p-2 text-gray-600 relative'>
                  <EllipsisVerticalIcon
                    ...
                    onClick={() => handleDropdownToggle(idx)} // Toggle for this row
                  />
                  {/* Dropdown Menu */}
                  {activeDropdown === idx && (
                    <div className='absolute right-32 mt-2 w-52 bg-white shadow-lg border rounded-md z-10'>
                      <ul className='text-gray-700'>
                        <li
                          className='px-4 py-2 flex gap-2 items-center cursor-pointer hover:bg-gray-100'
                          onClick={() => console.log('Insert Polygon Before')}
                        >
                        ...
                        <li
                          className='px-4 py-2 flex gap-2 items-center cursor-pointer hover:bg-gray-100'
                          onClick={() => console.log('Insert Polygon After')}
                        >
